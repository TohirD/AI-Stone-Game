...
function Points (totalStones, maxPlayer) { //calcuate the points for each player at the end of the game
    if (totalStones == 0) {
        return 0;  
    }
    if (totalStones % 2== 0) {
...
function CalculateScore (initialStones, path) { //evaluation function
    let score =0;
    let maxPlayer = true;
    let totalStones = initialStones;
    for (let i=0;i<gamePath.length;i++){ //calculate points for what has already happened
